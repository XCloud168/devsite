...
async function _createPayment(
  ...
  // 从收款地址池中获取一个可用的地址
  const availableAddress = await _getAvailableAddress(
    chain,
    lastPayment?.receiverAddress,
  );
  if (!availableAddress) {
 ...
 * 2. 如果找到，则返回该地址
 * 3. 如果找不到，则返回null
 * 4. 如果提供了address，则优先检查该地址是否可用，如果可用，则返回该地址，否则从地址池中获取
 *
 * @param chain - 链
 * @param address - 地址
 * @returns 收款地址
...
async function _getAvailableAddress(
  ...
  if (address) {
    availableAddress = await db.query.paymentAddresses.findFirst({
      where: (paymentAddresses, { eq, and, notExists, gte }) =>
        and(
